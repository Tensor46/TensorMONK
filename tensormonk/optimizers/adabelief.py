"""Code is from the url listed below, modified to avoid warnings and improve
readability.

URL: https://github.com/juntang-zhuang/Adabelief-Optimizer/blob
            /update_0.2.0/PyTorch_Experiments/imagenet/AdaBelief.py
"""
...
def zeros_like(tensor: torch.Tensor):
    if torch.__version__ < "1.5.0":
        return torch.zeros_like(tensor.data)
    return torch.zeros_like(tensor.data, memory_format=torch.preserve_format)
...
class AdaBelief(Optimizer):
    ...
    Arguments:
        ...
        amsgrad (boolean, optional): whether to use the AMSGrad variant of this
            ...
            (default: False)
        weight_decouple (boolean, optional): ( default: False) If set as True,
            then the optimizer uses decoupled weight decay as in AdamW
        fixed_decay (boolean, optional): (default: False) This is used when
            ...
            this case, the weight decay ratio decreases with learning rate.
        rectify (boolean, optional): (default: False) If set as True, then
    ...
    def reset(self):
        for group in self.param_groups:
            for p in group['params']:
                ...
                state['exp_avg_var'] = zeros_like(p)
                if amsgrad:
                    # Maintains max of all exp. moving avg. of sq. grad. values
                    state['max_exp_avg_var'] = zeros_like(p)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if amsgrad:
                    ...
                    max_exp_avg_var = torch.max(max_exp_avg_var, exp_avg_var)
                ...
                if not self.rectify:
                    ...
                    # calculate rho_t
                    state['rho_t'] = state['rho_inf'] - \
                        2 * state['step'] * beta2 ** state['step'] / \
