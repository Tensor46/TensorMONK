...
class CapsuleNet(nn.Module):
    ...
    def __init__(self, tensor_size=(6, 1, 28, 28), n_labels=10,
        ...
        if replicate_paper:
            ...
            block = Convolution
            self.InitialConvolutions = Convolution(tensor_size, filter_size=9,
                out_channels= 256, strides=1, pad=False, activation="relu")
            _tensor_size = self.InitialConvolutions.tensor_size
        else: # You can be creative!
            block = Convolution
            self.InitialConvolutions = nn.Sequential(
                Convolution(tensor_size, 5, 64, 1, False, "relu", 0., None, False),
        ...
        self.PrimaryCapsule = PrimaryCapsule(_tensor_size,
            filter_size=9, out_channels=256, strides=2, pad=False, activation="",
            dropout=0., batch_nm=False, pre_nm=False, block=block,
            n_capsules=primary_n_capsules, capsule_length=primary_capsule_length)
        ...
        self.Reconstruction = nn.Sequential(
            nn.Linear(n_labels*routing_capsule_length, 512), nn.ReLU(),
    ...
    def forward(self, tensor, targets):
        ...
        if self.training:
            ...
            rec_tensor = self.Reconstruction(
                (features * onehot_targets[:, :, None]).view(tensor_size[0], -1))
