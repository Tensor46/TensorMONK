...
class Convolution(nn.Module):
    ...
    def __init__(self, tensor_size, filter_size, out_channels, strides=(1, 1),
        ...
        if weight_nm:
            self.Convolution = nn.utils.weight_norm(nn.Conv2d(tensor_size[1]//pre_expansion,
                                                              out_channels*pst_expansion, filter_size, strides,
                                                              padding, bias=False, groups=groups, dilation=dilation),
                                                    name='weight')
        else:
            self.Convolution = nn.Conv2d(tensor_size[1]//pre_expansion, out_channels*pst_expansion,
                                         filter_size, strides, padding, bias=False, groups=groups, dilation=dilation)
            nn.init.kaiming_normal_(self.Convolution.weight, nn.init.calculate_gain("conv2d"))
            if equalized:
                import numpy as np
...
# test = Convolution((1,8,10,10), (3,3), 16, (2,2), True, "maxo", 0., "pixelwise", True)
# test(x).size()
