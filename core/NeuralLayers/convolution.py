...
class Convolution(nn.Module):
    ...
    def __init__(self,
        ...
        if not (activation is None or type(activation) == str):
            raise TypeError("Convolution: activation must None/str: "
        ...
        if not type(dropout) is float:
            raise TypeError("Convolution: dropout must float: "
        ...
        if not type(transpose) == bool:
            raise TypeError("Convolution: transpose must boolean: "
                            "{}".format(type(transpose).__name__))

        if not type(maintain_out_size) == bool:
            raise TypeError("Convolution: maintain_out_size must boolean: "
                            "{}".format(type(maintain_out_size).__name__))

        if not type(bias) == bool:
            raise TypeError("Convolution: bias must boolean: "
