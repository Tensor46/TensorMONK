...
class SEResidualComplex(nn.Module):
    ...
    def __init__(self, tensor_size, filter_size, out_channels, strides=(1, 1), pad=True,
                 activation="relu", dropout=0., batch_nm=False, pre_nm=False,
                 groups=1, weight_nm=False, r=16, *args, **kwargs):
...
class CarryModular(nn.Module):
    def __init__(self, tensor_size, filter_size, out_channels, strides=(1, 1), pad=True,
        ...
        if isinstance(block, torch.nn.modules.container.Sequential):
            ...
            self.network1 = block(tensor_size, filter_size, growth_rate, strides, pad, activation, 0., batch_nm, pre_nm, groups, weight_nm)
...
# test(x).size()
# %timeit test(x).size()
# test = CarryModular(tensor_size, 3, 128, 2, False, "relu", 0., False, False)
# test(x).size()
# %timeit test(x).size()
# test = SEResidualComplex(tensor_size, 3, 64, 1, False, "relu", 0., True, False)
# test(x).size()
# %timeit test(x).size()
# test = ResidualNeXt(tensor_size, 3, 64, 2, False, "relu", 0., True, False)
# test(x).size()
# %timeit test(x).size()
# test = SEResidualNeXt(tensor_size, 3, 64, 2, False, "relu", 0., True, False)
# test(x).size()
# %timeit test(x).size()
